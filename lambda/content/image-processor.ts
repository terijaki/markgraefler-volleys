...
const buildCompressedOriginalCommand = (
  inputPath: string,
  outputPath: string,
...
const processImage = async (bucket: string, uploadKey: string): Promise<Record<string, string>> => {
  ...
  try {
    ...
    // uploadKey: uploads/teams/uuid.jpg -> finalKey: teams/uuid.jpg
    ...
    try {
      if (sourceExtension === "gif") {
        ...
        while (quality >= 50 && !compressed) {
          try {
            await execAsync(
              buildCompressedOriginalCommand(
                inputImagePath,
            ...
            if (compressedSize <= MAX_ORIGINAL_SIZE) {
              ...
              console.warn(
                `Original image still exceeds limit of ${bytesToMB(MAX_ORIGINAL_SIZE, 0)}MB at quality 50 (${compressedSize} bytes)`,
              );
              compressed = true;
            }
          } catch (error) {
    ...
    for (const size of IMAGE_SIZES) {
      ...
      try {
        ...
        await execAsync(
          `convert "${inputImagePath}" -auto-orient -resize ${size} -background white -alpha remove -alpha off -quality ${IMAGE_QUALITY} -strip "${jpegPath}"`,
        );
      ...
      try {
        ...
        await execAsync(
          `convert "${inputImagePath}" -auto-orient -resize ${size} -quality ${IMAGE_QUALITY} -format webp -strip "${webpPath}"`,
        );
...
const lambdaHandler = async (event: S3Event): Promise<ProcessingJobResult[]> => {
  ...
  for (const record of event.Records) {
    try {
      ...
      // Example: teams/uuid-1234.jpg or members/abc-123/file.jpg

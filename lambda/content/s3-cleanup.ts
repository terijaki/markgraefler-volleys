/**
 * S3 Cleanup Lambda
 * Triggered by DynamoDB Streams from Teams, Members, Media, and Sponsors tables
...
import { mediaSchema, memberSchema, sponsorSchema, teamSchema } from "../../lib/db/schemas";
...
async function processRecord(record: DynamoDBRecord): Promise<void> {
  ...
  console.log(`Processing ${record.eventName} event from table`, {
  ...
  if (record.eventName === "REMOVE") {
    ...
    if (record.dynamodb.OldImage) {
      ...
      console.log(
        `REMOVE event: Found ${s3KeysToDelete.length} S3 keys to delete from old entity`,
        { keys: s3KeysToDelete },
      );
    }

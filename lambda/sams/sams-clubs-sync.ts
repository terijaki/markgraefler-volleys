  ...
  try {
    ...
    while (hasMorePages) {
      ...
      if (data.content) {
        ...
        if (association?.uuid) {
          ...
          console.log(
            `ℹ️ Association not found on page ${currentPage} (${data.content.length} Associations: ${data.content.map((a) => a.name).join(", ")}), checking next page...`,
          );
        }
        currentPage++;
      }
    ...
    if (!associationUuid) {
      ...
      if (data && data.name === ASSOCIATION_NAME) {
        ...
        console.log(
          `✅ Found association via workaround: ${ASSOCIATION_NAME} (${associationUuid})`,
        );
      }
    }
    ...
    while (hasMorePages) {
      ...
      if (data.content) {
        ...
        console.log(
          `📄 Fetched page ${currentPage + 1}/${data.totalPages} (${pageItems.length} clubs)`,
        );
        currentPage++;
      }
    ...
    Sentry.addBreadcrumb({
      category: "sync",
      message: `Total clubs fetched: ${totalWritten}`,
      level: "info",
      data: { clubsFetched: totalWritten },
    });
    ...
    Sentry.addBreadcrumb({
      category: "sync",
      message: "Clubs sync completed",
      level: "info",
      data: { clubsWritten: totalWritten, associationUuid },
    });

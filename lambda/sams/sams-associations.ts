...
const { logger, tracer } = createLambdaResources("sams-associations");
...
const lambdaHandler: APIGatewayProxyHandler = async (
  ...
  logger.info("Getting SAMS associations", { pathParameters: event.pathParameters });
  try {
    if (!SAMS_API_KEY) {
    ...
    if (name) {
      const desiredAssociation = allAssociations.find(

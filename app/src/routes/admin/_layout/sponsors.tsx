...
function CurrentLogoDisplay({
  ...
  const { data: logoUrl } = useQuery({
    queryKey: ["upload", "fileUrl", logoS3Key],
    queryFn: () => getFileUrlFn({ data: { s3Key: logoS3Key || "" } }),
    enabled: !!logoS3Key && !deleteLogo,
  });
  ...
  if (logoFile) {
    ...
    return (
      <Box>
        ...
        <Card withBorder p="md" bg="mvPurple" c="white">
  ...
  if (logoS3Key && !deleteLogo) {
    return (
      <Box>
        ...
        <Card withBorder p="md" bg="mvPurple">
          {logoUrl ? (
...
function SponsorsPage() {
  ...
  const createMutation = useMutation({
    ...
    onError: (error: unknown) => {
      setUploading(false);
      notification.error({
        message: (error as Error).message || "Sponsor konnte nicht erstellt werden",
      });
    },
  });
  const updateMutation = useMutation({
    ...
    onError: (error: unknown) => {
      setUploading(false);
      notification.error({
        message: (error as Error).message || "Sponsor konnte nicht aktualisiert werden",
      });
    },
  });
  const deleteMutation = useMutation({
    ...
    onError: (error: unknown) => {
      notification.error({
        message: (error as Error).message || "Sponsor konnte nicht gelöscht werden",
      });
    },
  });
  ...
  return (
    <Stack gap="md">
      ...
      <Modal
        opened={opened}
        onClose={close}
        title={editingId ? "Sponsor bearbeiten" : "Neuer Sponsor"}
...
function SponsorCard({
  ...
  const { data: logoUrl } = useQuery({
    queryKey: ["upload", "fileUrl", sponsor.logoS3Key],
    queryFn: () => getFileUrlFn({ data: { s3Key: sponsor.logoS3Key || "" } }),
    enabled: !!sponsor.logoS3Key,
  });
  ...
  return (
    <Card shadow="sm" padding="lg" radius="md" withBorder>
      <Card.Section withBorder bg="mvPurple">
        {logoUrl ? (
      ...
      <Stack gap="xs" mt="md">
        ...
        {sponsor.websiteUrl && (
          ...
          >
            <Group gap="xs">
              <Globe size={14} style={{ color: "var(--mantine-color-mvGreen-2)" }} />

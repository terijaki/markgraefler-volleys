...
export default function HomeTeamGrid({ teams }: { teams: Team[] }) {
  ...
  const disableAccordion = teams.length < 4;
      ...
      <Stack gap="md" justify="center">
        ...
        <Grid gap="md">
          {teamsSorted.map((team, index, array) => {
            ...
            const nextIsMatchingLeagueParticipation = leagueParticipation
              ? Boolean(nextTeam?.league)
              : true;
            ...
            return (
              <Fragment key={team.id}>
                <Grid.Col span={{ base: 12, sm: 12 / 2, lg: 12 / 3 }}>
                  <TeamCard {...team} disableAccordion={disableAccordion} />
                </Grid.Col>
                {lastSelectedTeam && (
                  <Grid.Col span={12}>
                    <Divider size="sm" color="mvSand.5" />
                  </Grid.Col>
                )}
              </Fragment>
            );

...
// Markgräfler Volleys 1 (Herren) — used for team-level bug checks
...
async function checkBug2(apiKey: string): Promise<BugResult> {
  ...
  try {
    ...
    const isFixed =
      data.logoImageForScreenOutputLink !== null && data.logoImageForScreenOutputLink !== undefined;
...
async function checkBug3(apiKey: string): Promise<BugResult> {
  ...
  try {
    ...
    const isFixed = entries.some(
      (e) => e.scoreIncludingLosses !== null && e.scoreIncludingLosses !== undefined,
    );
    return { id, summary, status: isFixed ? "fixed" : "still_present" };
  } catch (e) {
    return { id, summary, status: "check_failed", detail: String(e) };
...
// Heuristic: checks Markgräfler Volleys 1 whose shortName/clubCode are not explicitly set.
...
async function checkBug6(apiKey: string): Promise<BugResult> {
  const id = 6;
  const summary =
    "`date` field declared as `date-time` but API returns a date-only string (YYYY-MM-DD)";
  try {
    ...
    if (!mdRes.ok) {
      return {
        ...
        summary,
        status: "check_failed",
        detail: `HTTP ${mdRes.status} fetching match-days`,
      };
    }
    ...
    if (!matchRes.ok) {
      return {
        ...
        summary,
        status: "check_failed",
        detail: `HTTP ${matchRes.status} fetching matches`,
      };
    }
...
async function checkBug7(apiKey: string): Promise<BugResult> {
  const id = 7;
  const summary =
    "`referees`/`results` use `$ref + nullable: true` — invalid OpenAPI 3.0; breaks code generators";
  try {
    ...
    const matchRes = await samsGet(
      `/league-matches?size=20&for-league=${VERBANDSLIGA_HERREN_UUID}`,
      apiKey,
    );
    if (!matchRes.ok) {
      return {
        ...
        summary,
        status: "check_failed",
        detail: `HTTP ${matchRes.status} fetching league matches`,
      };
    }
    ...
    const hasNullField =
      matchData.content?.some((m) => m.referees === null || m.results === null) ?? false;
  ...
  try {
    ...
    if (!hierarchyRes.ok) {
      return {
        ...
        summary,
        status: "check_failed",
        detail: `HTTP ${hierarchyRes.status} fetching league-hierarchies`,
      };
    }
